...
export default function dataStreamer(ChartComponent) {
  ...
  class StreamingComponent extends React.Component {
    ...
    getData(offset) {
      let newData = this.props.data.slice(0, offset + 1 + LENGTH); // Experiment for updating same candle
      // if (offset > 0) {
  ...
  StreamingComponent.displayName = `dataStreamer(${getDisplayName(ChartComponent)})`;
